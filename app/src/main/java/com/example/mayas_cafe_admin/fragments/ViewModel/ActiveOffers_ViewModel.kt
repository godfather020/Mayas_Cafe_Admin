...
class ActiveOffers_ViewModel : ViewModel() {
    ...
    fun getActiveOffers(activity : Fragment, token : String, loading : ProgressBar) : MutableLiveData<Response_Common> {
        ...
        getActiveOffersFromAPI(request_branch)

        return commonResponse
    }

    private fun getActiveOffersFromAPI(param: Request_Branch) {

        val retrofitInstance: RetrofitInstance = RetrofitInstance()
        val retrofitData = retrofitInstance.retrofit.getOffers(token ,param)
