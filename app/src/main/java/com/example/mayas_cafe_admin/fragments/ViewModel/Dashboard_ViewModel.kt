...
class Dashboard_ViewModel : ViewModel() {
    ...
    fun getTotalProducts(activity : Fragment, token : String, loading : ProgressBar) : MutableLiveData<Response_Common>{

        this.activity = activity
        this.loading = loading
        this.token = token

        val requestProduct = Request_CategoryDetails()
        requestProduct.branchId = "1"

        getTotalProductsFromAPI(requestProduct)

        return commonResponse2
    }

    private fun getTotalProductsFromAPI(param: Request_CategoryDetails) {

        val retrofitInstance: RetrofitInstance = RetrofitInstance()
        val retrofitData = retrofitInstance.retrofit.getTotalProducts(token ,param)
